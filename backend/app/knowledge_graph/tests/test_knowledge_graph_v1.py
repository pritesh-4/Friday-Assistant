...
async def test_search_modes(graph, repository):
    ...
    repository.vector_store.search = AsyncMock(
        return_value=[{"id": "site_wiki", "distance": 0.1}]
    )
...
async def test_context_engine(graph, context_engine, repository):
    ...
    await graph.create_edge(
        ...
        "lang_python",
        "prefers",
        evidence="Explicitly stated python preference.",
    )
    ...
    assert (
        context["relevant_preferences"][0]
        == "User prefers 'python' (evidence: Explicitly stated python preference.)"
    )
