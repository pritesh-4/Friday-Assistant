...
class CognitiveMemoryService:
    ...
    async def save_extracted_memory(self, extracted: ExtractedMemory) -> None:
        ...
        existing = await self.retrieve_relevant_memories(
            extracted.content, limit_per_type=1
        )
        for docs in existing.values():
            if docs and docs[0].get("distance", 1.0) < 0.15:
                ...
                logger.info(
                    f"Duplicate memory skipped for {extracted.memory_type.value}: {extracted.content}"
                )
                return
        ...
        await database.execute(
            ...
            (
                ...
                memory_id,
                extracted.importance_score or 5,
                extracted.reason or "",
                now,
            ),
        )
    ...
    async def retrieve_relevant_memories(
        ...
        for mem_type in results.keys():
            ...
            docs = await vector_store.search(
                collection, query, n_results=limit_per_type
            )
