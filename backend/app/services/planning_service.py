...
class PlanningService:
    async def create_goal(self, goal_data: GoalBase) -> Goal:
        ...
        for m_idx, milestone in enumerate(goal_data.milestones):
            for t_idx, task in enumerate(milestone.tasks):
                ...
                task_id_map[task.title] = (
                    real_id  # Using title as mapping key for simplicity in LLM generation
                )
    ...
    async def get_goal(self, goal_id: str) -> Goal | None:
        ...
        milestone_rows = await database.fetch_all(
            "SELECT * FROM milestones WHERE goal_id = ? ORDER BY order_index ASC",
            (goal_id,),
        )
        milestones = []
        ...
        for m_row in milestone_rows:
            ...
            task_rows = await database.fetch_all(
                "SELECT * FROM planning_tasks WHERE milestone_id = ?", (m["id"],)
            )
            tasks = []
            for t_row in task_rows:
                ...
                # Fetch dependencies
                dep_rows = await database.fetch_all(
