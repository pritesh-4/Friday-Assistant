...
class PlannerAgent:
    ...
    async def plan_execution(
        self, messages: list[dict[str, Any]], available_agents: list[str]
    ) -> PlannerResponse:
        ...
        planner_messages = [{"role": "system", "content": system_instruction}]
        ...
        try:
            ...
            if (
                ...
                and agent_name not in available_agents
            ):
                logger.warning(
                    f"Planner suggested unknown agent '{agent_name}'. Falling back to conversational."
                )
                strategy = ExecutionStrategy.CONVERSATIONAL
                agent_name = None

            return PlannerResponse(
                strategy=strategy, agent_name=agent_name, reasoning=reasoning
            )

        except Exception as e:
            logger.error(
                f"Planner failed to parse intent, falling back to conversational: {e}"
            )
            return PlannerResponse(
                strategy=ExecutionStrategy.CONVERSATIONAL,
                agent_name=None,
